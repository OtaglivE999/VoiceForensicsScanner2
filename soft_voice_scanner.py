...
class SpeakerTrack:
    ...
    # Audio features for permanent storage
    ...
    voice_samples_count: int = 0  # Count for running average
    ...
    def update_audio_features(self, mfcc: np.ndarray, spectral_centroid: float,
                               spectral_bandwidth: float, zcr: float, 
                               pitch: float, energy: float):
        """Update running average of audio features for permanent storage."""
        n = self.voice_samples_count
        # Running average update
        if n == 0:
            self.mfcc_mean = mfcc.mean(axis=1) if mfcc.ndim > 1 else mfcc
            self.mfcc_std = mfcc.std(axis=1) if mfcc.ndim > 1 else np.zeros(13)
            self.spectral_centroid_mean = spectral_centroid
            self.spectral_bandwidth_mean = spectral_bandwidth
            self.zero_crossing_rate_mean = zcr
            self.pitch_mean = pitch
            self.energy_mean = energy
        else:
            # Incremental mean update
            alpha = 1.0 / (n + 1)
            mfcc_curr = mfcc.mean(axis=1) if mfcc.ndim > 1 else mfcc
            self.mfcc_mean = (1 - alpha) * self.mfcc_mean + alpha * mfcc_curr
            if mfcc.ndim > 1:
                self.mfcc_std = (1 - alpha) * self.mfcc_std + alpha * mfcc.std(axis=1)
            self.spectral_centroid_mean = (1 - alpha) * self.spectral_centroid_mean + alpha * spectral_centroid
            self.spectral_bandwidth_mean = (1 - alpha) * self.spectral_bandwidth_mean + alpha * spectral_bandwidth
            self.zero_crossing_rate_mean = (1 - alpha) * self.zero_crossing_rate_mean + alpha * zcr
            self.pitch_mean = (1 - alpha) * self.pitch_mean + alpha * pitch
            self.energy_mean = (1 - alpha) * self.energy_mean + alpha * energy
...
class SpeakerDatabase:
    ...
    def load(self):
        """Load speakers from files with audio features."""
        max_id = 0
            ...
            try:
                ...
                for fp in data.get('fingerprints', []):
                    if fp.get('embedding'):
                        ...
                        if 'audio_features' in fp:
                            af = fp['audio_features']
                            speaker.mfcc_mean = np.array(af.get('mfcc_mean', np.zeros(13)), dtype=np.float32)
                            speaker.mfcc_std = np.array(af.get('mfcc_std', np.zeros(13)), dtype=np.float32)
                            speaker.spectral_centroid_mean = float(af.get('spectral_centroid_mean', 0))
                            speaker.spectral_bandwidth_mean = float(af.get('spectral_bandwidth_mean', 0))
                            speaker.zero_crossing_rate_mean = float(af.get('zero_crossing_rate_mean', 0))
                            speaker.pitch_mean = float(af.get('pitch_mean', 0))
                            speaker.pitch_std = float(af.get('pitch_std', 0))
                            speaker.energy_mean = float(af.get('energy_mean', 0))
                            speaker.voice_samples_count = int(af.get('voice_samples_count', 0))
    ...
    def save(self):
        """Save database with full audio features."""
        fingerprints = []
        for sid, speaker in self.speakers.items():
            fingerprints.append({
                'id': speaker.id,
                ...
                # Audio features - MFCC and spectral
                'audio_features': {
                    'mfcc_mean': speaker.mfcc_mean.tolist() if isinstance(speaker.mfcc_mean, np.ndarray) else speaker.mfcc_mean,
                    'mfcc_std': speaker.mfcc_std.tolist() if isinstance(speaker.mfcc_std, np.ndarray) else speaker.mfcc_std,
                    'spectral_centroid_mean': float(speaker.spectral_centroid_mean),
                    'spectral_bandwidth_mean': float(speaker.spectral_bandwidth_mean),
                    'zero_crossing_rate_mean': float(speaker.zero_crossing_rate_mean),
                    'pitch_mean': float(speaker.pitch_mean),
                    'pitch_std': float(speaker.pitch_std),
                    'energy_mean': float(speaker.energy_mean),
                    'voice_samples_count': int(speaker.voice_samples_count)
                }
            })
        
        data = {
            'version': '2.2-full-features',
...
class SoftVoiceScanner:
    ...
    def extract_audio_features(self, audio: np.ndarray) -> Optional[dict]:
        """Extract MFCC and spectral features from audio segment."""
        try:
            import librosa
    ...
    def log_detection(self, speaker: SpeakerTrack, confidence: float,
        ...
        """Log to file with full audio features."""
        entry = {
            'timestamp': datetime.now().isoformat(),
        ...
        if audio_features:
            entry['audio_features'] = {
                'mfcc_mean': audio_features.get('mfcc_mean', []).tolist() if hasattr(audio_features.get('mfcc_mean', []), 'tolist') else audio_features.get('mfcc_mean', []),
                'mfcc_std': audio_features.get('mfcc_std', []).tolist() if hasattr(audio_features.get('mfcc_std', []), 'tolist') else audio_features.get('mfcc_std', []),
                'spectral_centroid': float(audio_features.get('spectral_centroid', 0)),
                'spectral_bandwidth': float(audio_features.get('spectral_bandwidth', 0)),
                'zero_crossing_rate': float(audio_features.get('zero_crossing_rate', 0)),
                'rms_energy': float(audio_features.get('rms_energy', 0)),
    ...
    def process_chunk(self, audio: np.ndarray):
        ...
        for start_sample, end_sample, is_soft in segments:
            ...
            # Extract audio features (MFCC, spectral, etc.) for permanent storage
            audio_features = self.extract_audio_features(segment_audio)
            ...
            if speaker:
                ...
                # Update speaker's audio features for permanent storage
                if audio_features:
                    speaker.update_audio_features(
                        mfcc=audio_features.get('mfcc', np.zeros((13, 1))),
                        spectral_centroid=audio_features.get('spectral_centroid', 0),
                        spectral_bandwidth=audio_features.get('spectral_bandwidth', 0),
                ...
                # Update new speaker's audio features
                if audio_features:
                    speaker.update_audio_features(
                        mfcc=audio_features.get('mfcc', np.zeros((13, 1))),
                        spectral_centroid=audio_features.get('spectral_centroid', 0),
            ...
            if speaker:
                ...
                results.append((speaker, confidence, snr, field_type, is_soft, audio_features))
                
                # Log detection with audio features
                self.log_detection(speaker, confidence, snr, field_type, is_soft, audio_features)
